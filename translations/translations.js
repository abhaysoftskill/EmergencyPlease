import LocalizedStrings from 'react-native-localization';
export const DEFAULT_LANGUAGE = 'English';

const translations = {
  'English': {
    WELCOME: 'Welcome !',
    CHANGELANGUAGE:'Change Language',
    ME:'Me',
    YOUAREHERE:'You are here',
    HELPLOCATION:'Help Location',
    SPLASHTEXT:'Emergency Please are always with you.',
    PLEASEWAIT:'Please Wait',
    ENTERMOBILENO:'Enter your 10 digit mobile number',
    GETSTARTED:'Get Started',
    USEPHONENUMBER:'Use Phone Number',
    USEEMAILID:'Use Email ID',
    ENTERPHONENUMBER:'Enter Phone Number',
    ENTEREMAILID:'Enter Email ID',
    LOGININ:'Login',
    YOURNAME:'Your Name',
    YOURLASTNAME:'Your Last Name',
    YOUREMAILID:'Your Email ID',
    ENTERYOURPASSWORD:'Enter Your Password',
    CONFIRMYOURPASSWORD:'Confirm Your Password',
    FORGOTPASSWORD:'Forgot Password',
    BACK:'Back',
    OTPSEND:'OTP SEND',
    OTPSENDTOMOBILE:'OTP sent to your registered mobile number',
    OTPRESEND:'Resend OTP',
    OTPVERIFY:'Verify OTP',
    USEPASSWORD:'Use Password',
    VALIDATE:'Validate',
    SIGNIN:'Sign IN',

  },
  'हिन्दी': {
    WELCOME: 'स्वागत हैं।',
    CHANGELANGUAGE:'भाषा बदलें',
    YOUAREHERE:'आप यहाँ हैं',

    SPLASHTEXT:'Emergency Please हमेशा आपके साथ हैं।',
    PLEASEWAIT:'कृपया प्रतीक्षा करें',

    ENTERMOBILENO:'अपना 10 अंकों का मोबाइल नंबर दर्ज करें',
    ENTEREMAILNO:'अपनी ईमेल आईडी दर्ज करें',
    GETSTARTED:'आगे जाओ',
    USEPHONENUMBER:'फोन नंबर का उपयोग करें',
    USEEMAILID:'ईमेल आईडी का प्रयोग करें',
    ENTERPHONENUMBER:'फोन नंबर दर्ज करें',
    ENTEREMAILID:'ईमेल आईडी दर्ज करें',
    LOGININ:'लॉग इन करें',
    YOURNAME:'आपका नाम',
    YOURLASTNAME:'आपका सरनेम',
    YOUREMAILID:'आपकी Email आईडी',
    ENTERYOURPASSWORD:'अपना Password डालें',
    CONFIRMYOURPASSWORD:'अपना Password कन्फर्म करें',
    FORGOTPASSWORD:'Password भूल गए',
    BACK:'वापस',
    OTPSEND:'OTP भेजें',
    OTPSENDTOMOBILE:'आपल्या नोंदणीकृत मोबाइल क्रमांकावर OTP पाठविला',
    OTPRESEND:'OTP पुनः भेजें',
    OTPVERIFY:'OTP मान्य करें',

    USEPASSWORD:'Password का उपयोग',
    VALIDATE:'मान्य करें',
    SIGNIN:'दाखिल करें',



  },
  'मराठी': {
    WELCOME: 'स्वागत आहे',
    CHANGELANGUAGE:'भाषा बदला',
    YOUAREHERE:'आपण येथे आहात',

    SPLASHTEXT:'Emergency Please आपल्याबरोबर नेहमीच असते.',
    PLEASEWAIT:'कृपया थांबा',

    ENTERMOBILENO:'आपला 10 अंकी मोबाइल नंबर प्रविष्ट करा',
    ENTEREMAILNO:'आपला ईमेल आयडी प्रविष्ट करा',
    GETSTARTED:'पुढे जा',
    USEPHONENUMBER:'फोन नंबर वापरा',
    USEEMAILID:'ईमेल आयडी वापरा',
    ENTERPHONENUMBER:'फोन नंबर प्रविष्ट करा',
    ENTEREMAILID:'ईमेल आयडी प्रविष्ट करा',
    LOGININ:'लॉगिन करा',
    YOURNAME:'आपले नाव',
    YOURLASTNAME:'आपले आडनाव ',
    YOUREMAILID:'आपला Email आयडी',
    ENTERYOURPASSWORD:'आपला Password प्रविष्ट करा',
    CONFIRMYOURPASSWORD:'आपल्या Password पुष्टी करा',
    FORGOTPASSWORD:'Password विसरलात?',
    BACK:'मागे',
    OTPSEND:'OTP पाठवा',
    OTPSENDTOMOBILE:'OTP sent to your registered mobile number',
    OTPRESEND:'OTP पुन्हा पाठवा',
    OTPVERIFY:'OTP प्रमाणित करा',
    USEPASSWORD:'Password वापरा',
    VALIDATE:'प्रमाणित करा',
    SIGNIN:'दाखिल करा',


  }
};

export default new LocalizedStrings(translations);